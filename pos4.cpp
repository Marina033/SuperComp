...
int main(int argc,char *argv[])
{
    ...
    double x1=-4, x2=4, y1=-1, y2=4;   
    double h1=(x2-x1)/N, h2=(y2-y1)/N;     
    double metrnorm=h1*h2;

    int num_threads=atoi(argv[1]);
    omp_set_num_threads(num_threads);
    double *deltas = new double [num_threads]();

    
    double time = omp_get_wtime();

    #pragma omp parallel for shared(a,x1,h1,y1,h2,metrnorm)  
    for (int i=1; i<=N; i++)
    {
        ...
        for (int j=1; j<=N; j++)
        {
            ...
            double yb=ya+h2;
            if (yb>=-yl) yb=min(yl,yb);
            else  
            {
              get_a(a,i,j)=1.0/metrnorm;   
               continue;
            }
            if (ya<=yl) ya=max(-yl,ya);
            else    
            {
               get_a(a,i,j)=1.0/metrnorm;
               continue;
    ...
    #pragma omp parallel for shared(b,x1,h1,y1,h2,metrnorm)
    for (int j=1; j<=N; j++)
    {
        ...
        for (int i=1; i<=N; i++)
        {
            ...
            double xb=xa+h1;
            if (xb>=-xl) xb=min(xl,xb);
            else  
            {
               get_b(b,i,j)=1.0/metrnorm;   
               continue;
            }
            if (xa<=xl) xa=max(-xl,xa);
            else    
            {
               get_b(b,i,j)=1.0/metrnorm;
               continue;
